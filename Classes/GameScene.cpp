...
GameScene::GameScene()
{
    CConfigMgr* theCfg = new CConfigMgr();
    theCfg->LoadScenceCfg("");
   
	mHudLayer = new HudLayer();
	this->addChild(mHudLayer, 1);
	mGameLayer = new GameLayer(theCfg);

	CGameLogic::GetInstance()->Layer(mGameLayer);
	CGameLogic::GetInstance()->HudLayer(mHudLayer);
